...
export default function Product() {
  ...
  const handleTriggerAddDialog = () => {
    ...
    if(!chosenVariant.quantity || chosenVariant.variant === null) {
      toast.error("Please select a quantity and variant")
      return
    }
    if(chosenVariant.quantity < 1 ||
      (chosenVariant.variant?.inStock && chosenVariant.quantity > parseInt(chosenVariant.variant?.inStock))
    ) {
          ...
          {chosenVariant.variant && (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-row gap-2">
                <Input
                  type="number"
                  value={chosenVariant.quantity || ""}
                  onChange={(e) => setChosenVariant({
                    ...chosenVariant,
                    quantity: parseInt(e.target.value)
                  })}
                  className="w-1/5 border-2 border-slate-300 focus:border-none p-2"
                />
                <Button
                ...
                <CartInfoDialog 
                  ...
                  onReset={() => setChosenVariant({variant: null, quantity: 1})}
                />
              </div>
            </div>
          )}
